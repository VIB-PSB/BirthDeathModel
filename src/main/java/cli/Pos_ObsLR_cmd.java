...
import workflows.positiveWGMs.PositiveWGM_ObservedLRT;
...
@Command(name = "posObs", description = "Observed LRT on positive WGMs", mixinStandardHelpOptions = true)
public class Pos_ObsLR_cmd implements Runnable{
    ...
    public void run(){
        ...
        try (PrintStream output = new PrintStream(new FileOutputStream(outputFile))) {
            ...
            // Create and execute the PositiveWGM_ObservedLRT instance
            PositiveWGM_ObservedLRT lrtDist = new PositiveWGM_ObservedLRT(treeFile, wgdFile, testedWGD, gfNumber, combinedOutputFile, gfCountsFile);
            lrtDist.execute();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
