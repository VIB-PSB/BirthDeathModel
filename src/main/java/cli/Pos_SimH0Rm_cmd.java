...
import workflows.positiveWGMs.PositiveWGM_H0Rm;
...
@Command(name = "posSimRm", description = "Simulated LRT with H0Rm on positive WGMs", mixinStandardHelpOptions = true)
public class Pos_SimH0Rm_cmd implements Runnable{
    ...
    public void run(){
        ...
        try (PrintStream output = new PrintStream(new FileOutputStream(outputFile))) {
            ...
            // Create and execute the PositiveWGM_H0Rm instance
            PositiveWGM_H0Rm lrtDist = new PositiveWGM_H0Rm(treeFile, wgdFile, testedWGD, gfNumber, combinedOutputFile, lrtOriginalFile);
            lrtDist.execute();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
