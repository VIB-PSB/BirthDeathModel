...
import workflows.negativeWGMs.NegativeWGM_H0Neg;
...
@Command(name = "negSimNeg", description = "Simulated LRT with H0Neg on negative WGMs", mixinStandardHelpOptions = true)
public class Neg_SimH0Neg_cmd implements Runnable{
    ...
    public void run(){
        ...
        try (PrintStream output = new PrintStream(new FileOutputStream(outputFile))) {
            ...
            // Create and execute the NegativeWGM_H0Neg instance
            NegativeWGM_H0Neg lrtDist = new NegativeWGM_H0Neg(treeFile, wgdFile, gfNumber, combinedOutputFile, lrtOriginalFile);
            lrtDist.execute();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
