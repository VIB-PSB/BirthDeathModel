...
public class ProbCalculator {
	...
	private double probCalcCache(double lambda, double t, int s, int c) {
		...
		if (cacheVal != null) {
			...
			// " times! Hit Ratio: " + ((double)cacheHit)/((double)(cacheHit +
			// cacheMiss)));
			// }
			return cacheVal;
		}
		cacheMiss++;
		// if ((cacheMiss % 10000) == 0) {
		// System.out.println("Cache missed " + cacheMiss +
		// " times! Hit Ratio: " + ((double)cacheHit)/((double)(cacheHit +
		// cacheMiss)));
		// System.out.println("Example miss: " + lambda + " " + t + " " + s +
		// " " + c);
		// }
		int b = Math.min(s, c);
	...
	//
	// double[][] pr = new double[1001][1001];
	//
	// for (int s = 1; s < 100; s++) {
	// for (int c = 1; c < 100; c++) {
	//
	// //insert the largets t here, to find the most strict bound on lambda:
	// trial for different lambdas
	//
	// // pr[s][c] = pc.probCalc(1.65,0.728748, s, c);
	//
	// /* The bound on alpha irrespective of lambda and t is 0.54, so for every
	// tree-file or GF-file or WGD-file this holds!*/
	// // pr[s][c] = pc.probCalcTwo(0.54,s, c);
	//
	// if (pr[s][c] < 0) {
	// System.out.println("negative prob: " + s + " " + c + " "
	// + pr[s][c]);
	//
	// pr[s][c] = 0;
	// }
	//
	// else if (pr[s][c] > 1) {
	// System.out.println("greater than 1 prob: " + s + " " + c
	// + " " + pr[s][c]);
	//
	// pr[s][c]=0;
	// }
